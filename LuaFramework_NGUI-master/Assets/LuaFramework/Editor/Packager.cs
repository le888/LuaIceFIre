...
public class Packager {
    ...
    static UnityEngine.Object LoadAssetForProject(string file)
    {
        if (file.EndsWith(".lua")) file += ".txt";
    ...
    static void HandleBundleUI(BuildTarget target)
    {
        ...
        string[] files = Directory.GetFiles("Assets/Prefabs/UI/", "*.prefab");
        foreach (var item in files)
        {
            //BuildPipeline.PushAssetDependencies();
            mainAsset = AssetDatabase.LoadMainAssetAtPath(item);
            
            assetfile = assetPath + getFileName(item) + AppConst.ExtName;
    ...
    static string getFileName(string str)
    {
        ...
        string fileName = str.Substring(0, str.LastIndexOf("Panel"));
    ...
    static void HandleExampleBundle(BuildTarget target) {
        ...
        ///------------------------------生成PopMessagePanel素材绑定-----------------------------------
        BuildPipeline.PushAssetDependencies();
        mainAsset = LoadAsset("PopMessage/Prefabs/PopMessagePanel.prefab");
        assetfile = assetPath + "popmessage" + AppConst.ExtName;
        BuildPipeline.BuildAssetBundle(mainAsset, null, assetfile, options, target);
        BuildPipeline.PopAssetDependencies();
        ...
        ///------------------------------生成LoginPanel素材绑定-----------------------------------
        BuildPipeline.PushAssetDependencies();
        mainAsset = LoadAssetForProject("UI/LoginPanel.prefab");
